...
@Test fun `Given same parameters calculateSteps returns the same result consistently`() {
    val result1 = calculateSteps(4, 3, 5)
...
@Test fun `Given impossible values then early return`() {
    ...
    assertEquals("The target can not be reached given the bottle sizes", result.steps[0])
}
